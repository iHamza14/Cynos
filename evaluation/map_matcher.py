...
class ViterbiSnapper:
    """Enterprise class definition for ViterbiSnapper."""
...
class SimpleRoadGraph:
        ...
        for line_id, line in enumerate(polylines):
            for i in range(len(line) - 1):
                self.segments.append(
                    {"id": f"road_{line_id}_seg_{i}", "a": line[i], "b": line[i + 1]}
                )
    ...
    def get_candidates(self, obs, radius=50.0):
        """Executes core logic for get_candidates."""
        candidates = []
...
if __name__ == "__main__":
    ...
    mock_road = [[(0.000, 0.000), (0.001, 0.000), (0.002, 0.000), (0.003, 0.000)]]
    ...
    for obs, matched in zip(noisy_observations, matched_path):
        print(
            f"Obs: {obs[0]:.5f}, {obs[1]:.5f} -> Snapped: {matched['lat']:.5f}, {matched['lon']:.5f} (Dist: {matched['dist_to_obs']:>5.1f}m)"
        )
