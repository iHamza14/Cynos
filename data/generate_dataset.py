...
VAL_FRACTION = 0.10
TEST_FRACTION = 0.10
MIN_MEAN_BLACKOUT_SPEED_KMH = (
    5.0  # retained as historical config; no longer rejects low-speed windows
)
...
SESSIONS = {
    "driver_a_s4": {
        "s_path": "Synchronised V abd S datasets/Categorised IOVNB Dataset/S (Driver A)/S4/S-S4.csv",
        "v_path": "Synchronised V abd S datasets/Categorised IOVNB Dataset/S (Driver A)/S4/V-S4.csv",
    },
    "driver_e_vw04": {
...
def find_col(df, *needles):
    ...
    hits = [
        c for c in df.columns if all(n.casefold() in str(c).strip().casefold() for n in needles)
    ]
    if len(hits) != 1:
        raise ValueError(
...
def label_road_segment(blackout_bins):
    ...
    yaw_active = bool(yaw_valid.size and np.percentile(yaw_valid, 75) >= CURVE_MIN_HEADING_RATE_DPS)

    tags = []
    duration = len(blackout_bins) / HZ
    if (
        net_change >= ROUNDABOUT_MIN_ABS_HEADING_CHANGE_DEG
        and duration >= ROUNDABOUT_MIN_DURATION_SEC
    ):
    ...
    if (
        np.isfinite(mean_abs_rate)
        and mean_abs_rate <= STRAIGHT_MAX_HEADING_RATE_DPS
        and not steering_active
    ):
...
def displacement_from_latlon(bins):
    """Executes core logic for displacement_from_latlon."""
...
def build_windows(bins, stride_sec=WINDOW_STRIDE_SEC):
    """Executes core logic for build_windows."""
    ...
    for start in range(0, len(bins) - context_n - blackout_n + 1, stride_n):
        ...
        if not all(np.all(np.isfinite(a)) for a in required_arrays) or not np.all(
            np.isfinite(required_gt)
        ):
        ...
        gps_eval = {
            ...
            "gps_orientation_deg": np.array(
                [b["mobile_gps_orientation"] for b in blk], dtype=float
            ),
        ...
        ground_truth = {
            ...
            # Direct synchronized vehicle yaw-rate target, in deg/s.
            # Project convention for phone prediction uses the negative vehicle rate.
            "yaw_rate_dps": -np.array([b["v_yaw_rate_dps"] for b in blk], dtype=float),
...
def select_nonoverlapping(windows, rng):
    """Executes core logic for select_nonoverlapping."""
    # Optimal interval scheduling: sort by end time to maximize non-overlapping count
    windows_sorted = sorted(
        windows, key=lambda w: w["metadata"]["window_start_sec"] + CONTEXT_SEC + BLACKOUT_SEC
    )
        ...
        pickle.dump(
            {
                ...
                "sessions": SESSIONS,
                "resampled_bins_by_session": bins_by_session,
