"""
Exports trained PyTorch models to ONNX format for mobile deployment.
"""
...
def export_models():
    ...
    gyro_ckpt = torch.load(
        "train/gyro_tcn_output/best_gyro_tcn_processed.pt", map_location=device, weights_only=False
    )
    ...
    dvse_model.load_state_dict(
        torch.load("train/dvse_output/best_dvse.pt", map_location=device, weights_only=False)
    )
    dvse_model.eval()
