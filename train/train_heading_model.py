...
from models.dvse_components import GyroTCN
import argparse
import pickle
import random
...
def parse_args():
    """Executes core logic for parse_args."""
...
def seed_everything(seed):
    """Executes core logic for seed_everything."""
    random.seed(seed)
    np.random.seed(seed)
...
def wrap_degrees(x):
    """Executes core logic for wrap_degrees."""
...
def extract_windows(items):
    """Executes core logic for extract_windows."""
    xs, ys, times = [], [], []
    skipped = 0
...
class WindowDataset(Dataset):
    """Enterprise class definition for WindowDataset."""
...
def integrated_heading_loss(pred, target, heading_weight):
    """Executes core logic for integrated_heading_loss."""
...
def main():
    ...
    train_loader = DataLoader(
        WindowDataset(x_train, y_train_n),
        batch_size=args.batch_size,
        shuffle=True,
        num_workers=args.workers,
    )
    test_loader = DataLoader(
        WindowDataset(x_test, y_test_n),
        batch_size=args.batch_size,
        shuffle=False,
        num_workers=args.workers,
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb in train_loader:
            xb, yb = xb.to(device), yb.to(device)
            pred = model(xb)
            loss, _, _ = integrated_heading_loss(pred, yb, args.heading_loss_weight)
    ...
    axes[0].plot(
        tt, noisy_yaw, label="noisy phone yaw rate", alpha=0.28, linewidth=1.0, linestyle="--"
    )
